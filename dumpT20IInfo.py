...
c.execute('create table teamT20ILive (t20iTeamId integer unique, startDate text, team text, opposition integer, location text, result integer, scoreLink text, rating real)')
...
for x in range(0, 12):
    ...
    while (i < len(data1)):
        ...
        team1LiveRating = {}
        c.execute('select rating from teamT20ILive where team=? order by t20iTeamId desc', (team1, ))
        ...
        team2LiveRating = {}        
        c.execute('select rating from teamT20ILive where team=? order by t20iTeamId desc', (team2, ))
        ...
        c.execute('insert or ignore into teamT20ILive (t20iTeamId, startDate, team, opposition, location, result, scoreLink, rating) values (?, ?, ?, ?, ?, ?, ?, ?)',
                  (t20iTeam1Id, None, team1, team2, None, result, None, team1LiveRating))
        c.execute('insert or ignore into teamT20ILive (t20iTeamId, startDate, team, opposition, location, result, scoreLink, rating) values (?, ?, ?, ?, ?, ?, ?, ?)',
                  (t20iTeam2Id, None, team2, team1, None, result, None, team2LiveRating))
        conn.commit()
    ...
    for i in range(0, t20iNum):
       ...
       c.execute('update teamT20ILive set startDate=?,location=?,scoreLink=? where t20iTeamId=?', (startDates[t20iIds[i]], locations[t20iIds[i]], scoreLinks[i], t20iTeam1Id))
       c.execute('update teamT20ILive set startDate=?,location=?,scoreLink=? where t20iTeamId=?', (startDates[t20iIds[i]], locations[t20iIds[i]], scoreLinks[i], t20iTeam2Id))
       conn.commit()
