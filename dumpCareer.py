...
c.execute('''create table battingTestCareer (startDate text, endDate text, playerId integer unique, player text, tests integer, innings integer, notOuts integer, runs integer, average real, strikeRate real,
          fifties integer, hundreds integer, dblHundreds integer, tripleHundreds integer, rating real)''')
c.execute('''create table bowlingTestCareer (startDate text, endDate text, playerId integer unique, player text, tests integer, innings integer, balls integer, runs integer, wickets integer, average real,
          strikeRate real, econRate real, fiveWkts integer, tenWkts integer, rating real)''')
c.execute('''create table allRoundTestCareer (startDate text, endDate text, playerId integer unique, player text, tests integer, runs integer, battingAverage real, hundreds integer, wickets integer,
          bowlingAverage real, fiveWkts integer, hundredFiveWkts integer, rating real)''')
...
for player in c.fetchall():
    ...
    if tests < 40 and tests >= 20 and rating != None: rating = rating * math.exp(-float(40-tests)/200)
    if tests < 20 and tests >= 10 and rating != None: rating = rating * math.exp(-float(40-tests)/100)
    if tests < 10 and rating != None: rating = rating * math.exp(-float(40-tests)/50)
    if tests < 5 and rating != None: rating = rating * math.exp(-float(40-tests)/25)    
    ...
    c.execute('''insert or ignore into battingTestCareer (startDate, endDate, playerId, player, tests, innings, notOuts, runs, average, strikeRate, fifties, hundreds, dblHundreds, tripleHundreds, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (startDate, endDate, player[0], player[1], tests, innings, notOuts, runs, battingAverage, strikeRate, fifties, hundreds, dblHundreds, tripleHundreds, rating))  
    ...
    c.execute('''insert or ignore into bowlingTestCareer (startDate, endDate, playerId, player, tests, innings, balls, runs, wickets, average, strikeRate, econRate, fiveWkts, tenWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (startDate, endDate, player[0], player[1], tests, innings, balls, runs, wkts, bowlingAverage, strikeRate, econRate, fiveWkts, tenWkts, rating))
    ...
    c.execute('''insert or ignore into allRoundTestCareer (startDate, endDate, playerId, player, tests, runs, battingAverage, hundreds, wickets, bowlingAverage, fiveWkts, hundredFiveWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (startDate, endDate, player[0], player[1], tests, runs, battingAverage, hundreds, wkts, bowlingAverage, fiveWkts, hundredFiveWkts, rating))    
    conn.commit()
conn.close()