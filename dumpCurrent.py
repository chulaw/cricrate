...
def dumpCurrentTeam(liveTable, currentTable, matchType):
    ...
    rating = {}
    startDate = {}
    currentTableStr = "select t.team, t.rating, t.startDate from "+liveTable+" t inner join(select team, max("+matchType+"TeamId) maxMatchId from "+liveTable+" group by team) tt on t.team = tt.team and t."+matchType+"TeamId = tt.maxMatchId and t.startDate>'"+date12mAgo+"' order by t.rating desc"    
    for row in c.execute(currentTableStr):
        rankDiff[row[0]] = pastRank[row[0]] - k if row[0] in pastRank else 0
        rank[row[0]] = k
        rating[row[0]] = row[1]
        startDate[row[0]] = row[2]
        k = k + 1        
    
    for team in rank:
        print(repr(team)+' '+repr(rankDiff[team])+' '+repr(rank[team])+' '+repr(rating[team])+' '+repr(bestCurrent[team]))
        c.execute("insert or ignore into "+currentTable+" (team, startDate, rankDiff, rank, rating, bestCurrentRating) values (?, ?, ?, ?, ?, ?)",
                  (team, startDate[team], rankDiff[team], rank[team], rating[team], bestCurrent[team]))
        conn.commit()
...
# c.execute('''create table teamTestCurrent (team text unique, startDate text, rankDiff integer, rank integer, rating real, bestCurrentRating text)''')
...
# c.execute('''create table teamODICurrent (team text unique, startDate text, rankDiff integer, rank integer, rating real, bestCurrentRating text)''')
...
# c.execute('''create table teamT20ICurrent (team text unique, startDate text, rankDiff integer, rank integer, rating real, bestCurrentRating text)''')
...
c.execute('''create table teamFT20Current (team text unique, startDate text, rankDiff integer, rank integer, rating real, bestCurrentRating text)''')
