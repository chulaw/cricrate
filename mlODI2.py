...
for  i in range(0, 2):
    ...
    # else:
    #     odi1 = pandas.read_csv("odiML" + `i` + ".csv")
    #     odi2 = pandas.read_csv("odiML" + `(i+1)` + ".csv")
    #     odi12.append(odi1)
    #     odi12.append(odi2)
    #     odi = pandas.concat(odi12)
    print "Innings: " + `(i+1)`
    odi = pandas.read_csv("odiML" + `(i+1)` + ".csv")

    odi.loc[odi["MatchOdds"] == "None", "MatchOdds"] = 0.5
    odi.loc[odi["MatchOddsAdj"] == "None", "MatchOddsAdj"] = 0.5
    odi_train = odi[odi['Id'] < 3315]
    odi_test = odi[odi['Id'] >= 3315]
    ...
    predictors = ["Runs", "Wkts", "Team1Rating", "Team2Rating", "Overs", "HomeAway", "BattingRating", "BowlingRating", "MatchOdds" , "MatchOddsAdj", "Momentum"] if i == 0 else ["RunsReq", "Wkts", "Team1Rating", "Team2Rating", "BallsRem", "HomeAway", "BattingRating", "BowlingRating", "MatchOdds", "MatchOddsAdj", "Momentum"]
    #predictors = ["RunsReq", "Wkts", "BallsRem", "Team1Rating", "Team2Rating", "HomeAway", "MatchOdds", "MatchOddsAdj"]

    for column in odi_train:
        #if(odi_train[column].dtype == np.float64 or odi_train[column].dtype == np.int64) and column not in ("Runs", "Wkts", "Team1Rating", "Team2Rating", "Overs", "HomeAway", "BattingRating", "BowlingRating", "MatchOdds" , "MatchOddsAdj", "Momentum", "Result", "BallsRem"):
        if(odi_train[column].dtype == np.float64 or odi_train[column].dtype == np.int64) and column not in ("Result"):
            odi_train[column] = (odi_train[column] - odi_train[column].mean()) / odi_train[column].std()
    ...
    scores = cross_validation.cross_val_score(alg, odi_train[predictors], odi_train["Result"], cv=3)
    print("Logisitic Regression: " + `scores.mean()`)
...
concatPreds.to_csv("odiMLPred.csv", index=False)
