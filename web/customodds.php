...
if(isset($_SESSION['screen_width']) AND isset($_SESSION['screen_height'])){
    ...
    header("Location: customodds.php?matchFormat=".$matchFormat."&inn=".$inn."&runs=".$runs."&overs=".$overs."&wkts=".$wkts."&startDate=".$startDate."&endDate=".$endDate."&battingTeam=".$battingTeam."&bowlingTeam=".$bowlingTeam);
} else {
...
<html>
<head>
    <title>cricrate | Custom Odds</title>
    <meta charset="UTF-8">
    ...
    function drawChartStacked() {
      var jsonData = $.ajax({
       url: "charts/customoddsColumn.php?matchFormat="+matchFormat+"&inn="+inn+"&runs="+runs+"&overs="+overs+"&wkts="+wkts+"&startDate="+startDate+"&endDate="+endDate,
       dataType:"json",
       async: false
       }).responseText;
      ...
      var showEvery = parseInt(data.getNumberOfRows() / 25);
        ...
        function drawChartBubble() {
           var jsonData = $.ajax({
         url: "charts/customoddsBubble.php?matchFormat="+matchFormat+"&inn="+inn+"&runs="+runs+"&overs="+overs+"&wkts="+wkts+"&startDate="+startDate+"&endDate="+endDate,
    	   dataType:"json",
    	   async: false
    	   }).responseText;
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                ...
                <ul class="nav navbar-nav navbar">
                    ...
                    <ul class="dropdown-menu">
                          ...
                          <li class="active"><a href="customodds.php">Custom<span class="sr-only">(current)</span></a></li>
                      </ul>
                    </li>
...
echo "<h3><b>Custom Match Scenario&nbsp;&nbsp;<small>&nbsp;&nbsp;<a href=\"blog.php?title=odds\">Methodology</a></small></b></h3>";
echo "<form class=\"form-inline\" role=\"form\" name=\"selectForm\" method=\"get\" action=\"customodds.php\">";
...
if ($matchFormat == "ODI") {
  if ($inn == 1) {
    if ($runs < 50) {
      $sql = 'select o.'.$matchFormatLower.'Id, avg(o.runs), avg(o.overs), avg(o.runRate), avg(o.wkts), o.teamBat, t.team1, t.team2, t.ground, t.startDate, avg(o.result) from overComparisonODI o, '.$matchFormatLower.'Info t where o.'.$matchFormatLower.'Id=t.'.$matchFormatLower.'Id and o.innings=1 and t.startDate>='.$startDate.' and t.startDate<='.$endDate.' and o.overs>='.($overs-1).' and o.overs<'.($overs+1).' and o.runs<=1 and o.wkts='.$wkts.' group by o.'.$matchFormatLower.'Id';
    } else {
...
echo "<li class=\"list-group-item\">";
if ($winOddsAdj > 55) {
  echo "<h4><b>Win Odds: <font color='green'>$winOddsText</font></b></h4>";
} elseif ($winOddsAdj <= 55 && $winOddsAdj >= 45) {
  echo "<h4><b>Win Odds: <font color='orange'>$winOddsText</font></b></h4>";
} else {
  echo "<h4><b>Win Odds: <font color='red'>$winOddsText</font></b></h4>";
}
...
while($res = $result->fetchArray(SQLITE3_NUM)) {
  ...
  echo "<td>".round($res[3], 2)."</td><td>".$res[4]."</td>";
  if ($matchFormat == "ODI") {
    if ($res[5] == $res[6]) {
