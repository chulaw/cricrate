...
for player in c.fetchall():
    ...
    c.execute('''insert or ignore into battingT20ICareer (startDate, endDate, playerId, player, innings, notOuts, runs, average, strikeRate, fifties, hundreds, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    endDate = c.fetchone()
    if endDate != None: endDate = endDate[0]
    c.execute('''insert or ignore into bowlingT20ICareer (startDate, endDate, playerId, player, innings, balls, runs, wickets, average, strikeRate, econRate, threeWkts, fiveWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    endDate = c.fetchone()
    if endDate != None: endDate = endDate[0]
    c.execute('''insert or ignore into allRoundT20ICareer (startDate, endDate, playerId, player, t20is, runs, battingAverage, fifties, wickets, bowlingAverage, threeWkts, thirtyTwoWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (startDate, endDate, player[0], player[1], t20is, runs, battingAverage, fifties, wkts, bowlingAverage, threeWkts, thirtyTwoWkts, rating))    
    conn.commit()    
conn.close()