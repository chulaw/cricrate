...
def inningsOdds(testId, innNum):
    ...
    for i in range(0, len(overComp)):
        ...
        oddsWinOdds.append(winOdds)
        oddsDrawOdds.append(drawOdds)
        oddsResult.append(matchResult)
        oddsOversRem.append(ballsRem / 6)
        #print `testId` + ' Innings: ' + `innNum` + ' Overs: ' + `overs` + ' Score: ' + `runs` + '/' + `wkts` + ' RunsReq: ' + `runsReq` + ' OversRem: ' + `(ballsRem/6)` + ' Result: ' + `matchResult` + ' Similar: ' + `similarCount` + ' Win: ' + `winOdds` + ' Draw: ' + `drawOdds`
        #c.execute('update overComparisonTest set winOdds=?,drawOdds=? where ocId=?', (winOdds, drawOdds, ocId))
        #conn.commit()

    fd = open('testOdds2.csv','a')
    for oo in range(len(oddsOvers)):
        print `testId` + "," + `innNum` + ","  + `oddsOvers[oo]` + "," + `oddsOversRem[oo]` + "," + `oddsWinOdds[oo]` + "," + `oddsDrawOdds[oo]` + "," + `oddsResult[oo]`
        fd.write(`testId` + "," + `innNum` + ","  + `oddsOvers[oo]` + "," + `oddsOversRem[oo]` + "," + `oddsWinOdds[oo]` + "," + `oddsDrawOdds[oo]` + "," + `oddsResult[oo]` + "\n")
    fd.close()
...
for x in range(0, len(result)):
    ...
    for i in range(0, len(overComp)):
        ...
        #print `testId` + ' Innings: 1 Overs: ' + `overs` + ' Score: ' + `runs` + '/' + `wkts` + ' Result: ' + `matchResult` + ' Similar: ' + `similarCount` + ' Win: ' + `winOdds` + ' Draw: ' + `drawOdds`
        #c.execute('update overComparisonTest set winOdds=?,drawOdds=? where ocId=?', (winOdds, drawOdds, ocId))
        #conn.commit()
    conn.close()
    fd = open('testOdds2.csv','a')
    for oo in range(len(oddsOvers)):
        print `testId` + ",1,"  + `oddsOvers[oo]` + "," + `oddsOversRem[oo]` + "," + `oddsWinOdds[oo]` + "," + `oddsDrawOdds[oo]` + "," + `oddsResult[oo]`
        fd.write(`testId` + ",1,"  + `oddsOvers[oo]` + "," + `oddsOversRem[oo]` + "," + `oddsWinOdds[oo]` + "," + `oddsDrawOdds[oo]` + "," + `oddsResult[oo]` + "\n")
    fd.close()
