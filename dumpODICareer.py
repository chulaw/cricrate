...
c.execute('''create table allRoundODICareer (startDate text, endDate text, playerId integer unique, player text, odis integer, runs integer, battingAverage real, fifties integer, wickets integer,
          bowlingAverage real, threeWkts integer, fiftyThreeWkts integer, rating real)''')
...
for player in c.fetchall():
    ...
    if startDate != None and endDate != None:
        ...
        longevity = (careerDays.days - noODIYears * 365 * 0.5) * (float(innings)/float(teamODIs)) / 16000
    if rating != None: rating = (rating + rating * longevity) * 1.086

    c.execute('''insert or ignore into battingODICareer (startDate, endDate, playerId, player, innings, notOuts, runs, average, strikeRate, fifties, hundreds, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    if startDate != None and endDate != None:
        ...
        endD = date(int(endDate[:4]), int(endDate[4:-2]), int(endDate[6:]))
        careerDays = endD - startD
        longevity = (careerDays.days - noODIYears * 365 * 0.5) * (float(innings)/float(teamODIs)) / 12500
    if rating != None: rating = (rating + rating * longevity) * 1.15

    c.execute('''insert or ignore into bowlingODICareer (startDate, endDate, playerId, player, innings, balls, runs, wickets, average, strikeRate, econRate, threeWkts, fiveWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    if startDate != None and endDate != None:
        ...
        endD = date(int(endDate[:4]), int(endDate[4:-2]), int(endDate[6:]))
        careerDays = endD - startD
        longevity = (careerDays.days - noODIYears * 365 * 0.5) * (float(odis)/float(teamODIs)) / 15000
    ...
    c.execute('''insert or ignore into allRoundODICareer (startDate, endDate, playerId, player, odis, runs, battingAverage, fifties, wickets, bowlingAverage, threeWkts, fiftyThreeWkts, rating)
              values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (startDate, endDate, player[0], player[1], odis, runs, battingAverage, fifties, wkts, bowlingAverage, threeWkts, fiftyThreeWkts, rating))    
    conn.commit()
conn.close()