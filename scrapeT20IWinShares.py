...
#set PYTHONIOENCT20ING=utf-8
conn = sqlite3.connect('ccrT20I.db')
c = conn.cursor()

# get t20is info
c.execute('select t20iId, startDate, scoreLink, result from t20iInfo')
t20isInfo = c.fetchall()
# ignoreDL = (47, 66, 72, 198, 246, 315, 317, 497, 679, 683, 738)
ignoreDL = (9, 26, 58, 60, 62, 68)
# t20iId2LastOverBowlerInn = {227:2, 575:2, 730:2, 731:2}
t20iId2LastOverBowlerInn = {59:2}
t20iId2LastOverBowler = {592:"Gordon Drummond"}
...
for x in range(startT20I, len(t20isInfo)):
    ...
    result =  t20isInfo[x][3]
    if t20iId in ignoreDL:
        continue
    print `t20iId` + "\n"

    c.execute('select startDate from t20iInfo where t20iId=?',(t20iId,))
    t20iDate = c.fetchone()
    t20iDate = t20iDate[0]

    c.execute('select batTeam, balls from detailsT20IInnings where t20iId=? and innings=1',(t20iId,))
    team1Balls = c.fetchone()
    team1 = team1Balls[0]
    inn1Balls = team1Balls[1]

    c.execute('select batTeam, balls from detailsT20IInnings where t20iId=? and innings=2',(t20iId,))
    team2Balls = c.fetchone()
    team2 = team2Balls[0]
    inn2Balls = team2Balls[1]

    c.execute('select rating from teamT20ILive where team=? and startDate<?',(team1, t20iDate))
    ...
    c.execute('select rating from teamT20ILive where team=? and startDate<?',(team2, t20iDate))
    ...
    c.execute('select runs from detailsT20IInnings where t20iId=? and innings=2',(t20iId,))
    ...
    c.execute('select player, playerId, runs, balls from battingT20IInnings where t20iId=? and innings=1',(t20iId,))
    batsmen1 = c.fetchall()
    batsmen1Names = []
    batsman2Id1 = {}
    ...
    c.execute('select player, playerId, runs, balls from battingT20IInnings where t20iId=? and innings=2',(t20iId,))
    batsmen2 = c.fetchall()
    batsmen2Names = []
    batsman2Id2 = {}
    batsman2Runs2 = {}
    ...
    notOutBatsman2Runs = {}
    notOutBatsman2Balls = {}
    c.execute('select player, playerId, runs, balls from battingT20IInnings where t20iId=? and notOut=1',(t20iId,))
    notOutB = c.fetchall()
    ...
    url = 'http://www.espncricinfo.com' + t20isInfo[x][2]
    print url
    ...
    for inn in range(1, 3):
        url = 'http://www.espncricinfo.com' + t20isInfo[x][2] + '?innings=' + `inn` + ';view=commentary'
        page = requests.get(url)
        tree = html.fromstring(page.text)
        ...
        for i in range(len(endOfOverInfo)):
            ...
            if "End of over " in endOfOverInfo[i]:
                ...
                # if t20iId == 183 and inn == 1 and over == 6:
                #      endOfOverBatsmen.insert(0, "R McLaren")
                #      endOfOverBatsmen.insert(1, "0 (0b)")
                #
                # if t20iId == 284 and inn == 2 and over == 9:
                #      endOfOverBatsmen.insert(0, "JEC Franklin")
                #      endOfOverBatsmen.insert(1, "0 (0b)")

                if t20iId == 29 and inn == 1 and over == 20:
                    endOfOverBatsmen.insert(0, "RP Singh")
                    endOfOverBatsmen.insert(1, "0 (0b)")

                if t20iId == 69 and inn == 2 and over == 13:
                    endOfOverBatsmen.insert(0, "MF Maharoof")
                    endOfOverBatsmen.insert(1, "0 (0b)")
                ...
                if lastBallDismissal == 0 and batsmanHasNotFaced == 0 and not (t20iId == 1 and inn == 2 and over == 16) and not (t20iId == 29 and inn == 2 and over == 20):
                ...
                for b in range(0, 6):
                    ball = b + 1
                    eventId = `t20iId` + `inn` + "'" + `(int(over)-1)` + "''" +`ball`+"'"
                    c.execute('select fielder, fielderId, droppedCatch, missedStumping, greatCatch, directHit, runsSaved, catch, stumping from fieldingEventT20I where eventId=?', (eventId,));
                    results = c.fetchall()
        ...
        totDismissedBallFaced = 0
        j = 0
        if t20iId in rainedStops:
            if inn == rainedStops[t20iId]: continue
        ...
        if over != 20 and lastBallDismissal == 0 and lastBallOfOverWin == 0 and batsmanHasNotFaced == 0:
            ...
            if len(dismissedPlayer) > 0:
                for k in range(0, len(dismissedPlayer)):
                    ...
                    if dismissedPlayer[k] == "Singh" and t20iId == 154:
            ...
            c.execute('select player from bowlingT20IInnings where t20iId=? and innings=? and balls % 6 <> 0',(t20iId, inn))
            lastOverB = c.fetchall()
            if len(lastOverB) == 1:
                lastOverBowlerName = lastOverB[0][0]
            elif t20iId in t20iId2LastOverBowlerInn:
                if t20iId2LastOverBowlerInn[t20iId] == inn:
                    lastOverBowlerName = t20iId2LastOverBowler[int(`t20iId` + `inn`)]
    ...
    for batsman in battingWinShares1:
        ...
        matchId = repr(int(t20iId)) + repr(batsmanId)
        print batsman + ": " + `totalWS` + ": " + `totalWSAdj`
        c.execute('insert or ignore into winSharesT20IMatch (matchId, playerId, player, t20iId, battingWS, bowlingWS, fieldingWS, totalWS, battingAdjWS, bowlingAdjWS, fieldingAdjWS, totalAdjWS) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (matchId, batsmanId, batsman, t20iId, battingWS, bowlingWS, fieldingWS, totalWS, battingWSAdj, bowlingWSAdj, fieldingWSAdj, totalWSAdj))
        
        c.execute('select matchId, battingRating, bowlingRating, fieldingRating, totalRating from winSharesT20ILive where playerId=? order by matchId desc', (batsmanId, ))
        ...
        c.execute('insert or ignore into winSharesT20ILive(matchId, startDate, playerId, t20iId, battingRating, bowlingRating, fieldingRating, totalRating) values ' '(?, ?, ?, ?, ?, ?, ?, ?)',
                (matchId, startDate, batsmanId, t20iId, battingLiveRating, bowlingLiveRating, fieldingLiveRating, totalLiveRating))
    ...
    for bowler in bowlingWinShares2:
        ...
        if bowler not in battingWinShares1:
            ...
            matchId = repr(int(t20iId)) + repr(bowlerId)
            print bowler + ": " + `bowlingWS` + ": " + `bowlingWSAdj`
            c.execute('insert or ignore into winSharesT20IMatch (matchId, playerId, player, t20iId, battingWS, bowlingWS, fieldingWS, totalWS, battingAdjWS, bowlingAdjWS, fieldingAdjWS, totalAdjWS) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                (matchId, bowlerId, bowler, t20iId, 0, bowlingWS, fieldingWS, totalWS, 0, bowlingWSAdj, fieldingWSAdj, totalWSAdj))
            
        c.execute('select matchId, battingRating, bowlingRating, fieldingRating, totalRating from winSharesT20ILive where playerId=? order by matchId desc', (bowlerId, ))
        liveRating = c.fetchone()
        if liveRating is None:
        ...
        c.execute('insert or ignore into winSharesT20ILive(matchId, startDate, playerId, t20iId, battingRating, bowlingRating, fieldingRating, totalRating) values ' '(?, ?, ?, ?, ?, ?, ?, ?)',
                (matchId, startDate, bowlerId, t20iId, battingLiveRating, bowlingLiveRating, fieldingLiveRating, totalLiveRating))
