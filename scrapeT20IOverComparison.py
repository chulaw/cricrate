...
ignoreDL = (11, 26, 70, 119, 124, 157, 159, 160, 242, 247, 252, 260, 270, 273, 290, 300, 318, 335, 340, 373, 380, 398)
  
# loop through t20i matches
#for x in range(0, 1):
for x in range(398, len(t20isInfo)):    
    t20iId = t20isInfo[x][0]
    if t20iId in ignoreDL:
        continue
    result = t20isInfo[x][7]    
    overComparisonURL = 'http://www.espncricinfo.com' + t20isInfo[x][11] + '?view=comparison'
    ...
    c.execute('select innings, batTeam, bowlTeam from detailsT20IInnings where t20iId=?', (t20iId,))        
    ...
    for i in range(len(rows)):
        if allOut1 == 0:
            ...
            ocId = `t20iId` + `1` + `int(overs)`
            print `ocId` + ' ' + `t20iId` + ' ' +  teamBat1 + ' ' + `runs1` + ' ' + `wkts1` + ' ' + `overRuns1` + ' ' + `runRate1` + ' ' + `result1`
            c.execute('''insert or ignore into overComparisonT20I (ocId, t20iId, innings, teamBat, overs, runs, wkts, overRuns, runRate, reqRate, runsReq, ballsRem, matchOdds, result)
                      values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                      (ocId, t20iId, 1, teamBat1, overs, runs1, wkts1, overRuns1, runRate1, None, None, None, None, result1))
            conn.commit()
        if (chase2 == 1 or allOut2 == 1) and (wkts1 != 10 and int(overs) != 20):
            ...
            if allOut1 == 0:
                ...
                ocId = `t20iId` + `2` + `int(overs)`
                print `ocId` + ' ' + `t20iId` + ' ' +  teamBat2 + ' ' + `runs2` + ' ' + `wkts2` + ' ' + `overRuns2` + ' ' + `runRate2` + ' ' + `reqRate2`  + ' ' + `runsReq2`  + ' ' + `ballsRem2` + ' ' + `result2`
                c.execute('''insert or ignore into overComparisonT20I (ocId, t20iId, innings, teamBat, overs, runs, wkts, overRuns, runRate, reqRate, runsReq, ballsRem, matchOdds, result)
                          values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                          (ocId, t20iId, 2, teamBat2, overs, runs2, wkts2, overRuns2, runRate2, reqRate2, runsReq2, ballsRem2, None, result2))
                conn.commit()
            else:
                if allOut1Row == 0:
                    allOut1Row = i
